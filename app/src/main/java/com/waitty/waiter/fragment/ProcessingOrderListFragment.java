...
public class ProcessingOrderListFragment extends Fragment implements getResponseData {
    ...
    public void onResume() {
        ...
        if(Utility.getSharedPreferencesBoolean(mContext, constant.PROCESSING_ORDER_RELOAD)){
            Utility.setSharedPreferencesBoolean(mContext,constant.PROCESSING_ORDER_RELOAD,false);
    ...
    private void getOrderItemDataAPI(boolean loaderShow) {
        try{
            ...
            Call<JsonElement> call = apiInterface.getProcessingOrder(jsonObject,Utility.getSharedPreferencesString(mContext, constant.USER_SECURITY_TOKEN));
            new APICall(mContext).Server_Interaction(call,this ,API.GET_PROCESSING_ORDER);
                ...
                switch (typeAPI) {
                    case API.GET_PROCESSING_ORDER:
                        ...
                        if(itemList.size()==0) {
                            ...
                            fragmentNeworderListBinding.txtNoRecord.setText(R.string.txt_no_processingorder);
    ...
    public void checkListSize(){
        if(itemList.size()==0)
            ((HomeActivity)mContext).pagerAdapterHome.statusHomeFragment.changeBackgroundColor(1,false);
        else
            ((HomeActivity)mContext).pagerAdapterHome.statusHomeFragment.changeBackgroundColor(1,true);
    }
}
